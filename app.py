...
st.set_page_config(layout="wide", initial_sidebar_state="collapsed",page_title="Podcast Recommender", page_icon=":headphones:")
...
def get_recommendations(podcasts_df, vectorizer, podcast_title):
    ...
    for i in sorted_indices:
        if i != index:
            recommendation = podcasts_df.iloc[i][['title', 'link', 'rating', 'image_url', 'text']].to_dict()
...
def get_recommendations2(keyword):
    ...
    tfidf = tfidf_vectorizer.fit_transform(podcasts_df['title'] + ' ' + podcasts_df['text'])
    ...
    recommended_podcasts = podcasts_df.iloc[top_podcasts_idx][['title', 'link', 'rating', 'image_url', 'text']].to_dict('records')
    ...
    if keyword:
        ...
        for i, recommendation in enumerate(recommendations):
            with cols[i%4]:
                ...
                st.markdown(f'<h6 style="margin-top: 10px;">{recommendation["title"]}</h6>', unsafe_allow_html=True)
...
st.markdown("<h2 style='font-family:monospace; font-size: 24px; font-weight: bold;'>Select a podcast:</h2>", unsafe_allow_html=True)

selected_podcast = st.selectbox('', options=podcasts_df.title.values, key=next(key_counter))

if st.button('Show Recommendations'):
    recommendations = get_recommendations(podcasts_df, vectorizer, selected_podcast)
...
def recommend_podcasts():
    ...
    filtered_podcasts_df = podcasts_df[podcasts_df['genre'].str.contains(selected_genres_str)]

    if len(filtered_podcasts_df) == 0:
        st.warning('No podcasts found for the selected genre(s). Please select different genre(s).')
    else:
        # Sort the podcasts by rating in descending order
        sorted_podcasts_df = filtered_podcasts_df.sort_values('rating', ascending=False)
        ...
        for i, (_, podcast) in enumerate(recommended_podcasts_df.iterrows()):
            with eval(f'col{i % 4 + 1}'):
                ...
                st.markdown(f'<h6 style="margin-top: 10px;">{podcast["title"]}</h6>', unsafe_allow_html=True)
                with st.container():
                    st.markdown(f'<p style="font-style: italic; margin-top: 5px;">{podcast["genre"]}</p>', unsafe_allow_html=True)
...
if recommend_button:
    if len(selected_genres) > 0:
        st.write(f"You have selected the following genres: {', '.join(selected_genres)}")
        recommend_podcasts()
    else:
